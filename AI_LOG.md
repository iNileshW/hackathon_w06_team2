...
|-------|--------|
| **Date** | 2026-06-25 |
| **Task** | Design a method to scientifically determine the best RAG chunk size `k` (and overlap and `n_results`) instead of using the guessed 400/80 defaults. |
| **What AI Generated** | `eval_chunks.py`: a hand-labelled gold set of 10 (query -> distinctive target passage) pairs drawn from the two policy docs; a sweep that, for each (chunk_size, overlap) config, re-chunks + re-embeds the policies into a throwaway in-memory Chroma collection and scores retrieval by hit-rate and MRR (mean reciprocal rank); a ranked results table; and a follow-up retrieval-depth sweep at the winning config. |
| **What You Changed + Why** | Scored on whether a retrieved chunk actually CONTAINS the labelled target passage, not just whether it came from the right file -- file-level matching would pass trivially (only two files) and tell us nothing about chunk size, whereas passage-level matching directly punishes a chunking that splits the relevant clause across a boundary, which is exactly the failure mode chunk size controls. Used isolated in-memory collections (no `persist_directory`) so the experiment never disturbs the app's `chroma_db`, and made it deterministic/repeatable so the recommendation is reproducible -- the "scientific" part. Added MRR alongside hit-rate because two configs can both find the passage but one ranks it first; MRR breaks that tie toward the config that retrieves the answer higher. The run showed the current 400/80 default (hit-rate 0.90, MRR 0.85) is beaten by chunk_size=600/overlap=0 (1.00, 0.95), and that `n_results=2` already saturates -- so the existing depth of 3 is safe. Left `indexer.py` unchanged: the task was to design the method, and applying the winner is a separate, reviewable change. |

## Instance 11: Incremental vector-store sync (design for scale)

| Field | Detail |
|-------|--------|
| **Date** | 2026-06-25 |
| **Task** | Design for scale -- make the system automatically keep the vector store in sync with new/changed/deleted policy docs instead of wiping and rebuilding the whole collection on every manual `index`. |
| **What AI Generated** | `sync_index()` in `indexer.py`: loads a `.index_manifest.json` recording each file's content hash + chunk count, then for each doc adds (new), upserts (hash changed), skips (unchanged), or deletes (removed) -- using stable `{filename}-{i}` ids so every op is idempotent. Plus `_upsert_file`, manifest load/save helpers, a `watch_and_sync()` polling loop as a dependency-free automatic trigger, and a `python main.py sync [--watch]` CLI command. |
| **What You Changed + Why** | Detected change by **content hash**, not mtime -- mtime flips on a no-op touch and is unreliable across copies/restores, whereas a hash only changes when the text actually does, so unchanged docs are never needlessly re-embedded (the whole point of scaling). Delete a file's old chunks before re-adding using `max(prev_chunks, new_chunks)` ids -- an edited doc can produce FEWER chunks, and without deleting the surplus the store would keep orphaned stale chunks. Kept `index_policies` (full rebuild) but had it seed the manifest so the two paths agree -- a full index followed by a sync correctly reports everything unchanged. Made `sync_index` return a summary dict and tested the whole lifecycle (add -> unchanged -> change -> delete) asserting search results track each state, since "it ran" doesn't prove the deleted doc actually left the index. Shipped `watch_and_sync` as a simple poll loop but documented it as a stand-in for the real triggers (watchdog observer / cron / upload webhook) so the trigger choice stays explicit rather than implied. |
